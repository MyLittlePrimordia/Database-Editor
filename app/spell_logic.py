...
SUGGEST_TIME_BUDGET = 0.4  # seconds for the on-demand full-dictionary scan
...
class SpellChecker:
    ...
    def _full_dict_scan(self, core, budget=SUGGEST_TIME_BUDGET):
        ...
        for w in self._known:
            checked += 1
            if checked % 50000 == 0 and time.time() - start > budget:
                break
    ...
    def suggest(self, word, limit=5):
        ...
        if len(dists) < limit and self._full_loaded:
            for w in self._full_dict_scan(core):
